...
const transactionIsolation = {
    ...
    // rec_version: readers use the latest committed record version instead of
    // waiting (indefinitely, waitTimeout=0) for concurrent uncommitted writers.
    // With the upstream no_rec_version default, any SELECT touching a row that
    // has an uncommitted newer version blocks until that transaction ends.
    ISOLATION_READ_COMMITTED           : [tpb.isc_tpb_read_committed, tpb.isc_tpb_rec_version],
    ISOLATION_REPEATABLE_READ          : [tpb.isc_tpb_concurrency],
    ISOLATION_SERIALIZABLE             : [tpb.isc_tpb_consistency],
    ISOLATION_READ_COMMITTED_READ_ONLY : [tpb.isc_tpb_read_committed, tpb.isc_tpb_rec_version],
    // Upstream no_rec_version variants, kept for callers that need the
    // blocking semantics explicitly.
    ISOLATION_READ_COMMITTED_NO_REC_VERSION : [tpb.isc_tpb_read_committed, tpb.isc_tpb_no_rec_version],
};
