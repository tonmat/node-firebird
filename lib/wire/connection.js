...
class Connection {
    ...
    _bind_events(host, port, callback) {
        ...
        self._socket.on('close', function() {
            ...
            // Operations in flight when the socket closed will never receive a
            // response. Fail their callbacks now: carrying them over to the
            // reconnected socket (old behaviour) left them waiting forever and
            // misaligned the FIFO response queue for every request sent after
            // the reconnect. _isClosed also makes _queueEvent and the
            // transaction/statement methods fail fast instead of writing to a
            // dead (or, during the reconnect window, null) socket.
            self._isClosed = true;
            var stranded = self._queue;
            self._queue = [];
            self._pending = [];
            if (stranded.length) {
                var strandedError = self.error || new Error('Connection lost while operations were pending.');
                stranded.forEach(function(strandedCb) {
                    doError(strandedError, strandedCb);
                });
            }

            // A pooled connection is not resurrected: with _isClosed set the
            // pool discards it on return/checkout and attaches a fresh one.
            // Reconnecting here would leave an orphaned server attachment,
            // since the pool may already have dropped this object.
            if (self._pooled) {
                return;
            }
        ...
        self._socket.on('data', function (data) {
            ...
            while (xdr.pos < xdr.buffer.length) {
                ...
                decodeResponse(xdr, cb, self, self._lowercase_keys, function (err, obj) {
                    ...
                    // synchronous throw here becomes an uncaughtException and
                    // kills the whole process. Fail the pending operation and
                    // drop the connection instead; the wire state is undefined
                    // after a decoder fault, so the buffer cannot be reused.
                    self.error = ex;
                    if (cb) {
                        self._queue.shift();
                        self._pending.shift();
    ...
    detach(callback) {
        ...
        if (self._isClosed) {
            // Socket already gone (unexpected close): the server-side
            // attachment no longer exists, so there is nothing to detach.
            // Complete the callback instead of leaving the caller hanging.
            if (callback)
                callback();
            return;
        }
    ...
    throwClosed(callback) {
        ...
        // Only emit when someone listens: an 'error' event on an EventEmitter
        // without listeners throws and would crash the process.
        if (this.db && this.db.listenerCount('error') > 0)
                    ...
                    switch (meta.type) {
                        ...
                        default:
                            switch (typeof value) {
                                ...
                                default:
                                    if (Buffer.isBuffer(value)) {
                                        ret[i] = new Xsql.SQLParamStringBuffer(value);
                                    } else {
        ...
        switch (r) {
            ...
            case Const.op_cont_auth:
                ...
                if (cnx._pendingAccept && (cnx._pendingAccept.pluginName === pluginName || Const.AUTH_PLUGIN_SRP_LIST.indexOf(pluginName) !== -1)) {
                    ...
                    if (!cnx.serverKeys) {
                        // The server may send op_cont_auth with an SRP plugin name
                        // and NO data: it is asking the client to (re)start the
                        // handshake with that plugin (e.g. after a plugin switch)
                        // instead of delivering salt+key. Re-send the client public
                        // key so the server replies with its salt and public key.
                        // The flag caps this at one restart per connection so a
                        // misbehaving server cannot loop the exchange forever.
                        if (!d.buffer || d.buffer.length === 0) {
                            if (!cnx._contAuthRestarted && cnx.clientKeys) {
                                cnx._contAuthRestarted = true;
                                cnx.sendOpContAuth(
                                    cnx.clientKeys.public.toString(16),
                                    Const.DEFAULT_ENCODING,
                                    pluginName
                                );
                                return; // wait for server salt+key
                            }
