...
exports.escape = function(value) {
  ...
  if (value instanceof Date)
    return '\'' + value.getFullYear() + '-' + (value.getMonth() + 1).toString().padLeft(2, '0') + '-' + value.getDate().toString().padLeft(2, '0') + ' ' + value.getHours().toString().padLeft(2, '0') + ':' +
        value.getMinutes().toString().padLeft(2, '0') + ':' + value.getSeconds().toString().padLeft(2, '0') + '.' + value.getMilliseconds().toString().padLeft(3, '0') + '\'';
...
SQLParamStringBuffer.prototype.encode = function(data) {
  if (this.value != null) {
    data.addStringBuffer(this.value);
    data.addInt(0);
  } else {
    data.addInt(1);
  }
